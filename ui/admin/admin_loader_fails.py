""" flask event handlers for the admin app, with Thos changes for sql-auth-admin (als apps, not wg/nginx).. kc apps break.
        * return minified app
...
def admin_events(flask_app: Flask, args: Args, validation_error: ValidationError):
    ...
    @flask_app.route("/01/auth-admin/<path:path>")
    def return_spa(path=None):
        """ Step 2 - return minified sra for default admin app
        fails: http://localhost:8080/realms/kcals/realms/kcals/protocol/openid-connect/auth?client_id=alsclient&redirect_uri=http%3A%2F%2Flocalhost%3A5656%2Fadmin-app%2Findex.html%23%2F%3F&state=f045f432-6486-442e-8961-69e89f9f202b&response_mode=fragment&response_type=code&scope=openid&nonce=267c9c62-9f71-4dc3-80f6-9129cbc7af73&code_challenge=q9hMzBqHg2bnl35VSV9ePwr8XnNnh4asFWz7y3-snGc&code_challenge_method=S256
        works: http://localhost:8080/realms/kcals/protocol/openid-connect/auth?client_id=alsclient&redirect_uri=http%3A%2F%2Flocalhost%3A5656%2Fadmin-app%2Findex.html%23%2F%3F&state=ded5c1b0-17f6-466b-ab9a-609778491751&response_mode=fragment&response_type=code&scope=openid&nonce=0ac339f8-911a-4bea-b3e8-978b0012d391&code_challenge=V1GMHwwxQ9k2TLAiBbTft3TB-cdNP786VPDkczv87bE&code_challenge_method=S256
        """
        global did_send_spa
        if path == "home.js":
            directory = "ui/admin"
    ...
    @flask_app.route('/01/ui/admin/<path:path>')
    def auth_admin_yaml(path=None):
        return send_from_directory('ui/admin/','authentication_admin.yaml')  # TODO - text substitutions for auth
    ...
    def admin_yaml(path=None):
        ...
        if use_type == "mem":
            ...
            if Config.SECURITY_ENABLED == False:
                ...
                elif getattr(Config.SECURITY_PROVIDER, 'auth_config', None):  #val re-added
