...
class Config:
    ...
    kc_base = os.getenv('APILOGICPROJECT_KEYCLOAK_BASE','http://localhost:8080')  # works

    # kc_base = 'http://localhost:8080'       # works for F5 [not https]
    # kc_base = 'http://172.19.0.2:8080'      # fails for F5 direct to docker...

    ''' keycloak location '''
    KEYCLOAK_REALM =  os.getenv('KEYCLOAK_REALM','kcals')
    KEYCLOAK_BASE = os.getenv('KEYCLOAK_BASE',f'{kc_base}')  # config.py - KEYCLOAK_BASE_URL: http://localhost:8080/realms/kcals
    ...
    print(f'config.py - KEYCLOAK_BASE_URL: {KEYCLOAK_BASE_URL}')
